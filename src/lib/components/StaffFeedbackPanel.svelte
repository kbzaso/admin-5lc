<script lang="ts">
	...
	$: recipientEmails = Array.isArray(campaign?.recipientEmails)
		...
		: [];
	$: recipientEmailsText = recipientEmails.join('\n');
	...
	<Card.Root>
		<Card.Header>
			...
			<Card.Description>
				...
				staff que trabajó ese día. Agrega un correo por línea (o separados por coma). No necesitan
				cuenta para responder, solo el enlace que reciben por correo. Una vez enviada la encuesta,
				preguntas y destinatarios quedan fijos.
			</Card.Description>
		</Card.Header>
		<Card.Content>
			<form
			...
			>
				...
				<label class="mt-4 flex flex-col gap-1">
					<span class="text-sm font-medium text-muted-foreground">
						Correos del staff (uno por línea)
					</span>
					<textarea
						...
						placeholder={'persona1@ejemplo.com\npersona2@ejemplo.com'}
						class="w-full rounded-md border bg-background px-3 py-2 text-sm disabled:opacity-60"
					></textarea>
				</label>
				{#if alreadySent}
					<p class="mt-4 flex items-center gap-2 text-sm text-muted-foreground">
						...
						La encuesta ya fue enviada; preguntas y destinatarios quedaron fijos.
					</p>
				{:else}
